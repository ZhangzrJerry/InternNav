...
def build_depthanythingv2(config):
    ...
    state_dict = torch.load(f'{MODEL_PATH_TO}/depth_anything_v2_metric_hypersim_vits.pth', map_location="cpu")
    model_state = DAv2_model.state_dict()
    filtered_state_dict = {k: v for k, v in state_dict.items() if k in model_state and v.shape == model_state[k].shape}
    DAv2_model.load_state_dict(filtered_state_dict, strict=False)
